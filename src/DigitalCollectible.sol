...
import { IDigitalCollectible } from "./interfaces/IDigitalCollectible.sol";
...
abstract contract DigitalCollectible is ZeroGasOwnable, ERC165, IDigitalCollectible {
    ...
    bytes32 public constant PERMIT_FOR_ALL_TYPEHASH = keccak256("PermitForAll(address spender,uint256 nonce,uint256 deadline)");
     ...
     */
    function __DC_init(string memory name_, string memory symbol_, address owner_) internal {
        require(!isAlreadyInit, "DC: Contract already init");
        isAlreadyInit = true;
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        ...
        require(owner != address(0), "DC: invalid token ID");
        return owner;
    }
    ...
    function permit(address spender, uint256 tokenId, uint256 deadline, bytes memory signature) external override {
        ...
        require(
            recoverAddress != address(0) && _isApprovedOrOwner(recoverAddress, tokenId)
                || SignatureChecker.isValidSignatureNow(ownerOf(tokenId), digest, signature),
            "DC: Invalid permit signature"
        );
    ...
    function permitForAll(address owner, address spender, uint256 deadline, bytes memory signature) external {
        ...
        (address recoverAddress,) = ECDSA.tryRecover(digest, signature);
        require(recoverAddress == owner || SignatureChecker.isValidSignatureNow(owner, digest, signature), "DC: Invalid permit signature");
    ...
    function _setApprovalForAll(address owner, address operator, bool approved) internal virtual {
        require(owner != operator, "DC: approve to caller");
    ...
    function _getPermitDigest(address spender, uint256 tokenId, uint256 nonce, uint256 deadline) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(PERMIT_TYPEHASH, spender, tokenId, nonce, deadline)));
    }
    ...
    function _getPermitForAllDigest(address spender, uint256 nonce, uint256 deadline) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(PERMIT_FOR_ALL_TYPEHASH, spender, nonce, deadline)));
    }

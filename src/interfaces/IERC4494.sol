...
/// @title EIP-4494 Permit for ERC-721 NFTs
/// @dev https://eips.ethereum.org/EIPS/eip-4494
interface IERC4494 is IERC165 {
    /// ERC165 bytes to add to interface array - set in parent contract
    ///
    /// _INTERFACE_ID_ERC4494 = 0x5604e225
    /// @dev Function to approve by way of owner signature
    /// @param spender the address to approve
    /// @param tokenId the index of the Collectible to approve the spender on
    /// @param deadline a timestamp expiry for the permit
    /// @param sig a traditional or EIP-2098 signature
    function permit(address spender, uint256 tokenId, uint256 deadline, bytes memory sig) external;

    /// @dev Returns the nonce of an Collectible - useful for creating permits
    /// @param tokenId the index of the Collectible to get the nonce of
